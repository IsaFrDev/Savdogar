...
class RegisterView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            logger.error(f"Database error in RegisterView: {e}")
            return Response({
                'error': 'Database is being initialized. Please try again in 30 seconds.'
...
class PasswordChangeView(APIView):
    ...
    @extend_schema(
        ...
        request=OpenApiExample('Password Change', value={'old_password': '...', 'new_password': '...'}),
    )
    def post(self, request):
        user = request.user
