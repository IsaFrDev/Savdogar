...
export function PWAInstallBanner({ language = 'en', variant = 'banner' }: PWAInstallBannerProps) {
    ...
    const labels = {
        ...
        subtitle: language === 'uz'
            ? "Savdoon'ni telefonga o'rnating — tezroq ishlaydi va oflayn rejim"
            : language === 'ru'
                ? 'Установите Savdoon на телефон — быстрее и оффлайн режим'
                : 'Install Savdoon on your device — faster experience & offline mode',
                ...
                <div className="flex items-center gap-4 p-5 rounded-2xl bg-emerald-500/10 border border-emerald-500/20">
                    ...
                    <div>
                        ...
                        <p className="text-[var(--text-dim)] text-xs mt-0.5">
                            {language === 'uz' ? "Savdoon ilovasi qurilmangizga o'rnatilgan" : language === 'ru' ? 'Приложение Savdoon установлено' : 'Savdoon app is installed on your device'}
                        </p>
                    </div>
                </div>
            );
