...
This plan covers three cross-cutting quality improvement tracks for the Bozorchi AI platform: Responsiveness (web + mobile), Internationalization (web frontend, mobile app, backend), and Security hardening. Tasks are organized by track and build incrementally so each step integrates cleanly into the previous one.
...
- [ ] 12. Implement Mobile App i18n system
  - [ ] 12.1 Create `mobile/src/i18n/index.ts` with `useTranslation` hook
    ...
    - `setLanguage` persists selection to `AsyncStorage` under key `'@bozorchi_language'`
...
- [ ] 13. Implement Backend language negotiation middleware
  - [ ] 13.1 Create `AcceptLanguageMiddleware` in `Bozorchi AI-backend/bozorchi/middleware.py`
...
- [ ] 16. Implement secret key and production config validators in `settings.py`
  - [ ] 16.1 Add `_validate_secret_key()` function to `Bozorchi AI-backend/bozorchi/settings.py`
    - Raise `ImproperlyConfigured` if `DJANGO_SECRET_KEY` is absent, empty, shorter than 50 characters, or matches a known weak value (including the `"bozorchi-vibrant-premium-ai-secure-key-2026-unique-production-ready"` placeholder and any key starting with `"django-insecure"`)
...
- [ ] 18. Remove the terminal endpoint and migrate to management commands
  - [ ] 18.1 Remove the `TerminalView` URL route from `Bozorchi AI-backend/bozorchi/urls.py`
    - Delete the URL pattern that maps to `TerminalView`
    - _Requirements: 15.1_

  - [ ] 18.2 Disable `TerminalView` in `Bozorchi AI-backend/bozorchi/terminal_views.py`
    - Remove or comment out the class body so it cannot be re-registered accidentally
    - _Requirements: 15.2_

  - [ ] 18.3 Create `Bozorchi AI-backend/bozorchi/management/commands/backup_db.py`
  ...
  - [ ] 18.4 Create `Bozorchi AI-backend/bozorchi/management/commands/db_stats.py`
    - Print model counts for key models
    - _Requirements: 15.4_

  - [ ] 18.5 Create `Bozorchi AI-backend/bozorchi/management/commands/clear_cache.py`
    - Call `cache.clear()` and print a confirmation message
    - _Requirements: 15.4_

  - [ ] 18.6 Create `Bozorchi AI-backend/bozorchi/management/commands/auth_sessions.py`
...
- [ ] 19. Add review spam and abuse protection
  - [ ] 19.1 Create `Bozorchi AI-backend/orders/throttles.py` with review throttle classes
...
- [ ] 21. Remove credential logging and extend `SensitiveDataFilter`
  - [ ] 21.1 Remove all `print()` statements from `Bozorchi AI-backend/accounts/views.py` `SuperAdminLoginView`
  ...
  - [ ] 21.2 Extend `SensitiveDataFilter` in `Bozorchi AI-backend/bozorchi/logging_filters.py`
  ...
  - [ ] 21.3 Update `LOGGING` configuration in `settings.py`
    ...
    - Ensure the `bozorchi` logger also routes through `mask_sensitive_data`
