...
This document describes the technical design for three cross-cutting quality improvement tracks on the **Bozorchi AI** multi-vendor e-commerce platform:
...
- **Backend**: Django 6 + DRF + PostgreSQL (`Bozorchi AI-backend/`)
...
- `setLanguage` persists the selection to `AsyncStorage` under the key `'@bozorchi_language'`.
...
```python
# Bozorchi AI-backend/bozorchi/middleware.py (new class)
...
```python
KNOWN_WEAK_KEYS = {
    'bozorchi-vibrant-premium-ai-secure-key-2026-unique-production-ready',
...
```python
# Bozorchi AI-backend/orders/throttles.py (new file)
...
The existing `SensitiveDataFilter` in `bozorchi/logging_filters.py` is extended to mask any `LogRecord` attribute or `args` dict key matching: `password`, `token`, `secret`, `key`, `credential` (case-insensitive). The `accounts` logger is added to `LOGGING['loggers']` with the `mask_sensitive_data` filter.
...
| i18n (mobile) | `AsyncStorage` key `'@bozorchi_language'` stores a `Language` string |
...
#### Backend Property Tests (`Bozorchi AI-backend/tests/`)
