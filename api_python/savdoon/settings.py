...
DEBUG = os.environ.get('DJANGO_DEBUG', 'False') == 'True'  # Default: False for security
...
CSRF_TRUSTED_ORIGINS = _origin_list + [
    "https://*.railway.app",
    "https://*.vercel.app",
]
...
INSTALLED_APPS = [
    ...
    'integrations',  # Payment & Marketplace Integrations
...
MIDDLEWARE = [
    ...
    'savdoon.security_middleware.FileUploadValidationMiddleware',
    'savdoon.security_middleware.SecurityHeadersMiddleware',
    'savdoon.security_middleware.SuspiciousActivityMiddleware',
]
...
if os.environ.get('REDIS_URL'):
    ...
    if not DEBUG:
        import warnings
        warnings.warn(
            'PRODUCTION WARNING: REDIS_URL is not set. '
            'WebSocket real-time features will NOT work correctly across multiple processes. '
            'Set REDIS_URL to a Redis connection string.',
            RuntimeWarning,
            stacklevel=2,
        )
    CHANNEL_LAYERS = {
...
if _redis_url:
    ...
    # Use LocMemCache locally to provide actual caching speed boost (Phase 1)
    CACHES = {
        'default': {
            'BACKEND': 'django.core.cache.backends.locmem.LocMemCache',
            'LOCATION': 'savdoon-local-cache',
        }
    }
...
DATABASES = {
    'default': dj_database_url.config(
        # Use DATABASE_URL env var (PostgreSQL in production/staging).
        # Falls back to SQLite only for local development when DATABASE_URL is not set.
...
# Fix for Windows PostgreSQL encoding issues (Cyrillic characters)
if DATABASES['default'].get('ENGINE') == 'django.db.backends.postgresql':
    DATABASES['default'].setdefault('OPTIONS', {})
    DATABASES['default']['OPTIONS']['client_encoding'] = 'UTF8'


# Warn loudly if running in production without PostgreSQL
if not DEBUG:
    _db_url = os.environ.get('DATABASE_URL', '')
    if not _db_url or _db_url.startswith('sqlite'):
        import warnings
        warnings.warn(
            'PRODUCTION WARNING: DATABASE_URL is not set or is SQLite. '
...
CORS_ALLOWED_ORIGINS = _origin_list
...
REST_FRAMEWORK = {
    ...
    'DEFAULT_THROTTLE_RATES': {
        'anon': '60/minute',
        'user': '300/minute',
        'login': '10/minute',   # used by auth views
        'ai': '20/minute',      # used by AI endpoints
    },
