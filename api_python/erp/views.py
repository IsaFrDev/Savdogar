...
from .models import (
    ...
    WarehouseTransferItem, ExpenseCategory, Expense, CustomerDebt, DebtTransaction
)
from .serializers import (
    ...
    CustomerDebtSerializer, DebtTransactionSerializer
)
...
class CustomerDebtViewSet(viewsets.ModelViewSet):
    ...
    ordering_fields = ['total_debt', 'last_transaction_at', 'created_at']

    def get_queryset(self):
        store_id = self.request.query_params.get('store_id')
        if store_id:
            return CustomerDebt.objects.filter(store_id=store_id)
        return CustomerDebt.objects.filter(store__owner=self.request.user)
...
class DebtTransactionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        debt_id = self.request.query_params.get('debt_id')
        if debt_id:
            return DebtTransaction.objects.filter(debt_id=debt_id)
        return DebtTransaction.objects.filter(debt__store__owner=self.request.user)
