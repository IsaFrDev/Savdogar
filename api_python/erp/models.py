...
class CustomerDebt(models.Model):
    """Tracking credit/debt for customers"""
    store = models.ForeignKey('stores.Store', on_delete=models.CASCADE, related_name='customer_debts')
    customer_name = models.CharField(max_length=255)
    customer_phone = models.CharField(max_length=20, blank=True)
    customer = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name='debts')
    
    total_debt = models.DecimalField(max_digits=12, decimal_places=2, default=0)
    limit = models.DecimalField(max_digits=12, decimal_places=2, default=0, help_text="Maximum allowed debt")
    ...
    def __str__(self):
        return f"{self.customer_name} - {self.total_debt}"


class DebtTransaction(models.Model):
    """Individual debt increase/decrease transactions"""
    TRANSACTION_TYPES = [
        ('increase', 'Debt Increase (Credit)'),
        ('decrease', 'Debt Decrease (Payment)'),
    ]
    
    debt = models.ForeignKey(CustomerDebt, on_delete=models.CASCADE, related_name='transactions')
    amount = models.DecimalField(max_digits=12, decimal_places=2)
    ...
    def __str__(self):
        return f"{self.debt.customer_name}: {self.amount} ({self.transaction_type})"
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new:
            # Update total debt
            if self.transaction_type == 'increase':
                self.debt.total_debt += self.amount
            else:
                self.debt.total_debt -= self.amount
            self.debt.save()
