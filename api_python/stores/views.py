...
class StoreViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        ...
        # SuperAdmin only actions for explicit approval/rejection
    ...
    def get_queryset(self):
        ...
        # Superadmins see all stores
        if user.is_superuser or getattr(user, 'role', '') == 'superadmin':
            return Store.objects.all().order_by('-created_at')
        
        if not user.is_authenticated:
            # Public users only see approved, active, not in maintenance stores
            return Store.objects.filter(
                status='approved', maintenance_mode=False
            ).order_by('-rating', '-created_at')
            
        # Store owners see their own stores (including draft/rejected/maintenance)
    ...
    def approve(self, request, pk=None):
        ...
        try:
            ...
            # Update store status and set initial subscription
            store.status = 'approved'
            # The field `is_deleted` does not exist in the `Store` model.
            # store.is_deleted = False # Ensure it's not marked as deleted if re-approved
            ...
            # Invalidate cache
            cache.delete(f"store_data_{store.slug}")
    ...
    def reject(self, request, pk=None):
        ...
        Upon rejection, the store is deactivated.
        """
        store = self.get_object()
        ...
        # Update store status and deactivate
        store.status = 'rejected'
        # The field `is_deleted` does not exist in the `Store` model.
        # store.is_deleted = True # Automatically deactivate/soft-delete the store
        store.save()
        
        # Also remove associated contracts and signature data for a clean rejection
        ...
        store.save() # Save again after contract changes
        ...
        return Response({
            'message': 'Store rejected and deactivated',
            'store': StoreSerializer(store).data
        })
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        # The field `is_deleted` does not exist in the `Store` model.
        # Original logic for `is_deleted` has been removed. Access control for 'deleted' stores
        # should now rely on the `status` field (e.g., 'rejected').
        # if instance.is_deleted and not (request.user.is_superuser or getattr(request.user, 'role', '') == 'superadmin'):
        #     return Response({'error': 'Store not found or deleted'}, status=status.HTTP_404_NOT_FOUND)
    ...
    @extend_schema(
        ...
        description='Create a new store with contract signature. Stores are created as "draft" and are manageable by owner immediately.',
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            # Immediately allow management: set status to 'draft'
            # The field `is_deleted` does not exist in the `Store` model.
            store = serializer.save(status='draft') 
            
            try:
                from .store_templates import get_template_for_business_type
                template = get_template_for_business_type(store.business_type)
                
                req_primary = data.get('primary_color') or template['primary_color']
                req_secondary = data.get('secondary_color') or template['secondary_color']
                req_accent = template['accent_color']
                
                store_html = template['html'].replace('{{STORE_NAME}}', store.name)
                store_css = template['css'].replace('{{PRIMARY_COLOR}}', req_primary)
                store_css = store_css.replace('{{SECONDARY_COLOR}}', req_secondary)
                store_css = store_css.replace('{{ACCENT_COLOR}}', req_accent)
                ...
                store.theme_config = {
                    'primary_color': req_primary,
                    'secondary_color': req_secondary,
                    'accent_color': req_accent,
                }
                store.primary_color = req_primary
                store.secondary_color = req_secondary
                store.save()
                
                logger.info(f"Store {store.id} created with template from database: {template['name']}")
            except Exception as template_error:
                logger.warning(f"Template generation failed for store {store.id}: {template_error}")
            ...
            # Notify superadmins about new store creation
            try:
                ...
                for admin in superadmins:
                    Notification.objects.create(
                        user=admin,
                        type='store_created',
                        title='Yangi do\'kon yaratildi',
                        title_uz='Yangi do\'kon yaratildi',
                        title_ru='Создан новый магазин',
                        message=f'"{store.name}" do\'konini tasdiqlash kutilmoqda.',
                        message_uz=f'"{store.name}" do\'konini tasdiqlash kutilmoqda.',
                        message_ru=f'Ожидается подтверждение магазина "{store.name}".',
                        store=store,
                        read=False
                    )
            except Exception as notify_err:
                logger.warning(f"Failed to notify admins about new store: {notify_err}")
            ...
            # Re-raise validation errors so the frontend gets field-level details
            if isinstance(e, DRFValidationError):
                raise
            return Response({
                'error': "Do'kon yaratishda xatolik yuz berdi. Iltimos, qayta urinib ko'ring."
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def update(self, request, *args, **kwargs):
        ...
        cache_key = f"store_data_{instance.slug}"
    ...
    def by_slug(self, request):
        try:
            ...
            cache_key = f"store_data_{slug}"
            cached_data = cache.get(cache_key)
            
            if cached_data:
                # Still check maintenance mode for cached data
                if cached_data.get('maintenance_mode'):
                    user = request.user
                    is_privileged = user.is_authenticated and (user.is_superuser or getattr(user, 'role', '') == 'superadmin' or cached_data.get('owner_id') == user.id)
            ...
            try:
                # Ensure only non-deleted stores are fetched by slug for public access
                # The field `is_deleted` does not exist in the `Store` model.
            ...
            # If not approved, only show basic info or indicate pending/draft/rejected state
            if store.status not in ['approved', 'draft'] and store.owner != request.user:
            ...
            store_data['owner_id'] = store.owner_id  # Store owner id for cache validation
            cache.set(cache_key, store_data, timeout=60 * 15)  # Cache for 15 minutes
    ...
    def marketplace(self, request):
        ...
        try:
            ...
            # If authenticated, also show their own stores (even if draft/rejected)
            # The field `is_deleted` does not exist in the `Store` model.
    ...
    def download_contract(self, request, pk=None):
        ...
        # Ensure only owner or superadmin can download contract
        if request.user != store.owner and not (request.user.is_superuser or getattr(request.user, 'role', '') == 'superadmin'):
            return Response({'error': 'Permission denied'}, status=status.HTTP_403_FORBIDDEN)
    ...
    def nearby(self, request):
        ...
        # Filter for approved, not in maintenance stores for nearby search
        # The field `is_deleted` does not exist in the `Store` model.
...
class PendingStoresView(generics.ListAPIView):
    """List all stores awaiting approval for super admin (i.e., 'draft' status)."""
    serializer_class = StoreSerializer
    permission_classes = [IsAuthenticated, IsSuperAdmin]
    def get_queryset(self):
        # Admin reviews stores in 'draft' status
        # The field `is_deleted` does not exist in the `Store` model.
        return Store.objects.filter(status='draft').order_by('-created_at')
...
class StoreApprovalView(APIView):
    ...
    def post(self, request, pk):
        try:
            # Ensure admin can manage even 'draft' or 'rejected' stores
        ...
        if action == 'approve':
            store.status = 'approved'
            # The field `is_deleted` does not exist in the `Store` model.
            # store.is_deleted = False # Ensure it's active
            store.subscription_expiry = timezone.now() + timezone.timedelta(days=30)
            message = 'Store approved'
            # Send notification to store owner for approval
            Notification.objects.create(
                user=store.owner,
                type='store_approval',
                title='Do\'koningiz tasdiqlandi!',
            ...
            store.signature_data = ""
            store.contracts.all().delete()
            # Send notification to store owner for rejection
            reason = request.data.get('reason', '') # Assuming reason is passed for rejection
            reason_uz = request.data.get('reason_uz', '')
            reason_ru = request.data.get('reason_ru', '')
            ...
            from savdoon.notification_service import notification_service
            # This line might fail if telegram_bot_token is not set or valid, consider try-except
            try:
                if store.telegram_bot_token and store.telegram_chat_id:
                    notification_service.send_telegram_message(store.telegram_bot_token, store.telegram_chat_id, f"❌ Do'kon rad etildi: {reason}")
            except Exception as e:
                logger.error(f"Failed to send Telegram rejection notification for store {store.id}: {e}")
        store.save()
        # Invalidate cache
        cache.delete(f"store_data_{store.slug}")
...
class AcknowledgeRejectionView(APIView):
    ...
    def post(self, request):
        user = request.user
        # Find a rejected store owned by the user
        # The field `is_deleted` does not exist in the `Store` model.
        rejected_store = Store.objects.filter(owner=user, status='rejected').first()
        if not rejected_store:
            return Response({'error': 'No rejected store found that needs acknowledgment'}, status=status.HTTP_404_NOT_FOUND)
        
        # Optionally, if the user explicitly acknowledges, we might want to permanently delete the store record
        # For now, just revert user role and leave the soft-deleted store as is.
        user.role = 'customer'
        user.save()
...
class StoreSubResourceMixin:
    ...
    def get_store(self):
        ...
        try:
            # When getting a store for sub-resources, ensure it's not deleted (or rejected)
            # The field `is_deleted` does not exist in the `Store` model.
            store = Store.objects.get(pk=store_id)
            # Further logic might be needed here to filter out 'rejected' stores if they should not be accessible by sub-resource views.
        except Store.DoesNotExist:
            from rest_framework.exceptions import NotFound
            raise NotFound('Store not found')
            
        user = self.request.user
        # Allow if user is superadmin OR if they are the owner
        is_owner = store.owner == user
...
class BranchViewSet(StoreSubResourceMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Branch.objects.filter(store_id=self.kwargs['store_id'], is_active=True)


class StoreBannerViewSet(StoreSubResourceMixin, viewsets.ModelViewSet):
    serializer_class = StoreBannerSerializer
    def get_queryset(self):
        return StoreBanner.objects.filter(store_id=self.kwargs['store_id'], is_active=True)


class StaffRoleViewSet(StoreSubResourceMixin, viewsets.ModelViewSet):
    serializer_class = StaffRoleSerializer
    def get_queryset(self):
        return StaffRole.objects.filter(store_id=self.kwargs['store_id'], is_active=True)
...
class StaffMemberViewSet(StoreSubResourceMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        qs = StaffMember.objects.filter(store_id=self.kwargs['store_id'], is_active=True)
        if staff_type:
